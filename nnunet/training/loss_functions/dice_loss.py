...
class DC_and_topk_loss(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        if self.aggregate == "sum":
            ...
            raise NotImplementedError("nah son") # reserved for other stuff (later?)
        return result
class DC_and_CE_list_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1):
    ...
    def forward(self, net_output, target):
        ...
        if isinstance(net_output, list):
            num = len(net_output)
            weight = torch.tensor([0.5 ** (n + 1) for n in range(num)]).cuda()
            for n in range(num):
                output = net_output[n]
                pool_size = [int(u / v) for u,v in zip(target.shape[-3:],output.shape[-3:])]
                tt = torch.max_pool3d(target, pool_size, pool_size)
                dc_loss = self.dc(output, tt) if self.weight_dice != 0 else 0
                ce_loss = self.ce(output, tt) if self.weight_ce != 0 else 0
                result += (self.weight_ce * ce_loss + self.weight_dice * dc_loss) * weight[n]
            return result
